...
df_distinct_geom <- df_sf %>% dplyr::select(ogc_fid,codesource_area) %>% dplyr::filter(ogc_fid %in% list_area_id$ogc_fid)  %>% filter(!st_is_empty(.)) # %>% dplyr::select(codesource_area)
...
# default_wkt <- st_as_text(st_as_sfc(st_bbox(df_sf)))
# wkt(default_wkt)
