server <- function(input, output, session) {
  ...
  data_pie_all_datasets <- eventReactive(input$submit, {
    st_read(con, query = paste0("SELECT dataset, sum(measurement_value) as measurement_value FROM (",sql_query(),") AS foo GROUP BY  dataset"))
  ...
  data_i2 <- eventReactive(input$submit, {
    st_read(con, query = paste0("SELECT measurement_unit, gear_type AS gear_type, year, species, sum(measurement_value) as measurement_value FROM (",sql_query(),") AS foo GROUP BY measurement_unit, gear_type, year, species")) 
  },
  # on.exit(dbDisconnect(conn), add = TRUE)
  ignoreNULL = FALSE)
